...
describe('Test interface functionality', () => {
  ...
  test('Test interface commands [2]', (done) => {
    ...
    const daemon = new Interface(daemonsCopy);
    const requests = [['getblocktemplate', []]];
    const expected = [{'data': '{"error":true,"result":null,"instance":"nocktest"}', 'error': true, 'instance': { 'host': '127.0.0.1', 'port': '8332', 'username': 'foundation', 'password': 'foundation', 'index': 0 }, 'response': 'Unauthorized RPC access. Invalid RPC username or password'}];
    daemon.sendCommands(requests, false, (response) => {
      expect(response).toStrictEqual(expected);
      done();
    });
  ...
  test('Test interface commands [6]', (done) => {
    const daemon = new Interface(daemonsCopy);
    const expected = {'data': null, 'error': true, 'instance': null, 'response': 'No commands passed to daemon'};
  ...
  test('Test interface commands [7]', (done) => {
    ...
    const daemon = new Interface(daemonsCopy);
    const requests = [['getblocktemplate', []]];
    const expected = [{'data': 'null', 'error': true, 'instance': { 'host': '127.0.0.1', 'port': '8332', 'username': 'foundation', 'password': 'foundation', 'index': 0 }, 'response': 'Could not parse RPC data from daemon response' }];
    daemon.sendCommands(requests, false, (response) => {
      expect(response).toStrictEqual(expected);
      done();
    });
  ...
  test('Test interface commands [8]', (done) => {
    ...
    const daemon = new Interface(daemonsCopy);
    const requests = [['getblocktemplate', []]];
    const expected = [{'data': 'blajahahge', 'error': true, 'instance': { 'host': '127.0.0.1', 'port': '8332', 'username': 'foundation', 'password': 'foundation', 'index': 0 }, 'response': 'Could not parse RPC data from daemon response' }];
    daemon.sendCommands(requests, false, (response) => {
      expect(response).toStrictEqual(expected);
      done();
    });
  ...
  test('Test interface commands [9]', (done) => {
    const daemon = new Interface(daemonsCopy);
    const requests = [['getblocktemplate', []]];
    const expected = {'data': null, 'error': true, 'instance': { 'host': '127.0.0.1', 'port': '8332', 'username': 'foundation', 'password': 'foundation', 'index': 0 }, 'response': 'connect ECONNREFUSED 127.0.0.1:8332'};
    daemon.sendCommands(requests, true, (response) => {
      expect(response).toStrictEqual(expected);
      done();
    });
